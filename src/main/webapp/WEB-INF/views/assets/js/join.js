function idCheck() {
	...
	$("#id_uniq_check").click(function() {
		...
		// valid() 함수를 사용하여 id="user_id"인 input의 값만 유효성 검사 실시
		// 통과 = true / 실패 = false
		var idVaild = $("#user_id").valid();

		// valid()의 return값이 true면 ajax 요청 시작
		if (idVaild == true) {
			var userId = $('#user_id').val();

			// 중복 아이디 검사를 위한 controller로 ajax 요청
			$.ajax({
				url : "userIdCheck?userId=" + userId,
				method : "GET",
				success : function(json) {
					// 0 == 중복된 아이디가 DB에 존재하지 않음
					if (json.item == 0) {
						swal({
							text : '사용 가능한 아이디입니다.',
							type : 'success',
						...
						// 버튼(중복검사 <-> 수정하기) 교체
						$('#id_uniq_check').css("display", "none");
...
function joinVali() {
	...
	$("#join_form").validate({
		...
		messages : {
			...
			tel : "연락처 형식이 잘못되었습니다.",
			join_agree : "약관에 동의해 주세요."
		},
		// validation 통과 후 실시되는 submit 막기
		submitHandler : function(frm) {
			if ($('#ckd_id').val() == 1) {
				swal("", "아이디 중복검사가 필요합니다.", "warning");
				return;
			}
			if ($('#ckd_email').val() == 1) {
				swal("", "이메일주소 중복검사가 필요합니다.", "warning");
				return;
			}
			swal({
				text : "회원가입을 완료하시겠습니까?",
...
function daumPostcode() {
	new daum.Postcode({
		oncomplete : function(data) {
			...
			// 상세주소 readonly 해제
			$("#detailAddress").attr("placeholder", "상세주소를 입력하세요.").attr(
					"readonly", false)
